...
void CheckClass::constructors()
{
    ...
    for (std::size_t i = 0; i < classes; ++i) {
        ...
        for (func = scope->functionList.begin(); func != scope->functionList.end(); ++func) {
			...
			//Constructor declaration and Constructor definition 
			if (Token::Match(func->tokenDef->next(),"( ) { }")||Token::Match(func->token->next(),"( ) { }"))
			{
				continue;
			}
			//end
			...
			std::vector<Usage>::iterator Usageitr;
			unsigned int all = 0;
			for (Usageitr = usage.begin(); Usageitr != usage.end(); Usageitr++)
			{
				if (Usageitr->init == true || Usageitr->assign == true)
				{
					all++;
				}
			}

			if (all>0 && all<usage.size() && varname != "")
			{
				uninitVarError(func->token, scope->className, varname, inconclusive);
			}
...
void CheckClass::copyConstructorMallocError(const Token *cctor, const Token *alloc, const std::string& varname)
{
    ...
    reportError(callstack, Severity::warning, ErrorType::None, "copyCtorNoAllocation", "Copy constructor does not allocate memory for member '" + varname + "' although memory has been allocated in other constructors.");
}
*/

void CheckClass::copyConstructorShallowCopyError(const Token *tok, const std::string& varname)
{
	reportError(tok, Severity::style, ErrorType::None, "copyCtorPointerCopying", "Value of pointer '" + varname + "', which points to allocated memory, is copied in copy constructor instead of allocating new memory.", ErrorLogger::GenWebIdentity(tok->str()));
}
...
void CheckClass::noCopyConstructorError(const Token *tok, const std::string &classname, bool isStruct)
{
    ...
    reportError(tok, Severity::style, ErrorType::None, "noCopyConstructor",
                "'" + std::string(isStruct ? "struct" : "class") + " " + classname +
				"' does not have a copy constructor which is recommended since the class contains a pointer to allocated memory.", ErrorLogger::GenWebIdentity(tok->str()));
}
...
void CheckClass::initializeVarList(const Function &func, std::list<const Function *> &callstack, const Scope *scope, std::vector<Usage> &usage, bool bListOnly)
{
    if (!func.functionScope)
...
void CheckClass::noConstructorError(const Token *tok, const std::string &classname, bool isStruct)
{
    ...
    reportError(tok, Severity::style, ErrorType::None, "noConstructor",
				...
				"instantiated. That may cause bugs or undefined behavior.", ErrorLogger::GenWebIdentity(tok->str()));
}
...
void CheckClass::noExplicitConstructorError(const Token *tok, const std::string &classname, bool isStruct)
{
    ...
    reportError(tok, Severity::style, ErrorType::None, "noExplicitConstructor", message + "\n" + verbose, ErrorLogger::GenWebIdentity(classname));
}

void CheckClass::uninitVarError(const Token *tok, const std::string &classname, const std::string &varname, bool inconclusive)
{
    reportError(tok, Severity::warning, ErrorType::Uninit, "uninitMemberVar", "Member variable '" + classname + "::" + varname + "' is not initialized in the constructor.", 
		0U, inconclusive, ErrorLogger::GenWebIdentity(classname + "::" + varname));
}

void CheckClass::operatorEqVarError(const Token *tok, const std::string &classname, const std::string &varname, bool inconclusive)
{
    reportError(tok, Severity::warning, ErrorType::None, "operatorEqVarError", "Member variable '" + classname + "::" + varname + "' is not assigned a value in '" + classname + "::operator='.",
		0U, inconclusive, ErrorLogger::GenWebIdentity(classname + "::" + varname));
}
...
{
    reportError(tok, Severity::performance, ErrorType::None, "useInitializationList", "Variable '" + varname + "' is assigned in constructor body. Consider performing initialization in initialization list.\n"
				...
				"could avoid assigning '" + varname + "' a value by passing the value to the constructor in the initialization list.", ErrorLogger::GenWebIdentity(tok->str()));
}
...
{
	reportError(tok, Severity::style, ErrorType::None, "unusedPrivateFunction", "Unused private function: '" + classname + "::" + funcname + "'", ErrorLogger::GenWebIdentity(tok->str()));
}
...
void CheckClass::mallocOnClassWarning(const Token* tok, const std::string &memfunc, const Token* classTok)
{
    ...
    reportError(toks, Severity::warning, ErrorType::None, "mallocOnClassWarning",
...
void CheckClass::mallocOnClassError(const Token* tok, const std::string &memfunc, const Token* classTok, const std::string &classname)
{
    ...
    toks.push_back(tok);
    toks.push_back(classTok);
    reportError(toks, Severity::error, ErrorType::None, "mallocOnClassError",
...
{
    reportError(tok, Severity::error, ErrorType::None, "memsetClass",
				...
				"is created.",  ErrorLogger::GenWebIdentity(tok->str()));
}

void CheckClass::memsetErrorReference(const Token *tok, const std::string &memfunc, const std::string &type)
{
	reportError(tok, Severity::error, ErrorType::None, "memsetClassReference", "Using '" + memfunc + "' on " + type + " that contains a reference.", ErrorLogger::GenWebIdentity(tok->str()));
}

void CheckClass::memsetErrorFloat(const Token *tok, const std::string &type)
{
    reportError(tok, Severity::portability, ErrorType::None, "memsetClassFloat", "Using memset() on " + type + " which contains a floating point number.\n"
				...
				" Note: In case of an IEEE754-1985 compatible implementation setting all bits to zero results in the value 0.0.", ErrorLogger::GenWebIdentity(tok->str()));
}
...
{
    reportError(tok, Severity::style, ErrorType::None, "operatorEq", "'" + className + "::operator=' should return '" + className + " &'.\n"
...
{
	reportError(tok, Severity::style, ErrorType::None, "operatorEqRetRefThis", "'operator=' should return reference to 'this' instance.", ErrorLogger::GenWebIdentity(tok->str()));
}

void CheckClass::operatorEqShouldBeLeftUnimplementedError(const Token *tok)
{
	reportError(tok, Severity::style, ErrorType::None, "operatorEqShouldBeLeftUnimplemented", "'operator=' should either return reference to 'this' instance or be declared private and left unimplemented.", ErrorLogger::GenWebIdentity(tok->str()));
}

void CheckClass::operatorEqMissingReturnStatementError(const Token *tok, bool error)
{
    if (error) {
		reportError(tok, Severity::error, ErrorType::None, "operatorEqMissingReturnStatement", "No 'return' statement in non-void function causes undefined behavior.", ErrorLogger::GenWebIdentity(tok->str()));
    } else {
...
{
    reportError(tok, Severity::warning, ErrorType::None, "operatorEqToSelf",
				...
				"allocated memory is owned and managed by only one instance of the class.", ErrorLogger::GenWebIdentity(tok->str()));
}
...
void CheckClass::virtualDestructorError(const Token *tok, const std::string &Base, const std::string &Derived, bool inconclusive)
{
    if (inconclusive)
		...
		reportError(tok, Severity::error, ErrorType::MemoryLeak, "virtualDestructor", "Class '" + Base + "' which is inherited by class '" + Derived + "' does not have a virtual destructor.\n"
...
{
	reportError(tok, Severity::warning, ErrorType::Suspicious, "thisSubtraction", "Suspicious pointer subtraction. Did you intend to write '->'?", ErrorLogger::GenWebIdentity(tok->str()));
}
        ...
        for (func = scope->functionList.begin(); func != scope->functionList.end(); ++func) {
            ...
            if (func->type == Function::eFunction && func->hasBody() && !func->isFriend() && !func->isStatic() && !func->isVirtual()) {
                ...
                if (Token::Match(previous, "*|&")) {
                    ...
                    if (opName.compare(8, 5, "const") != 0 && *opName.rbegin() == '&')
                        continue;
                } else {
...
void CheckClass::checkConstError2(const Token *tok1, const Token *tok2, const std::string &classname, const std::string &funcname, bool suggestStatic)
{
    ...
    if (tok2)
        toks.push_back(tok2);
    if (!suggestStatic)
        reportError(toks, Severity::style, ErrorType::None, "functionConst",
        ...
        reportError(toks, Severity::performance, ErrorType::None, "functionStatic",
...
void CheckClass::initializerListError(const Token *tok1, const Token *tok2, const std::string &classname, const std::string &varname)
{
    ...
    toks.push_back(tok1);
    toks.push_back(tok2);
    reportError(toks, Severity::style, ErrorType::None, "initializerList",
...
{
	reportError(tok, Severity::error, ErrorType::None, "selfInitialization", "Member variable '" + varname + "' is initialized by itself.", ErrorLogger::GenWebIdentity(tok->str()));
}
    ...
    const std::string &purefuncname)
{
    ...
    reportError(tokStack, Severity::warning, ErrorType::None, "pureVirtualCall", "Call of pure virtual function '" + purefuncname + "' in " + scopeFunctionTypeName + ".\n"
        ...
        const std::string &variablename, bool derivedIsStruct, bool baseIsStruct)
{
    ...
    reportError(toks, Severity::warning, ErrorType::None, "duplInheritedMember", message, 0U, false);
}
